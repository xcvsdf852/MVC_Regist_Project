...
<html>
 ...
 <div class="container">
     <form class="form-signin" role="form" id="form1" name="form1" action="/homework0721_MVC/Account/check_user" method="post">
         ...
         <input type="text" name="txtUserName" id="txtUserName" class="form-control" placeholder="輸入帳號" autofocus="" value = "<?php if(is_null($data['error']['account'])|| !isset($data['error'])){echo "";}else{echo $data['error']['account'];} ?>"> 
         <label for="inputPassword" class="sr-only">輸入密碼</label> 
             <input type="password" name="txtPassword" id="txtPassword"  class="form-control" placeholder="輸入密碼" value ="<?php if(is_null($data['error']['pwd'])|| !isset($data['error'])){echo "";}else{echo $data['error']['pwd'];} ?>">
